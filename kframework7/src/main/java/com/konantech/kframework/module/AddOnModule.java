package com.konantech.kframework.module;
...
public class AddOnModule {
    ...
    // 공통
    @Value("#{kframework['addon.addr']}") private String addr;
    @Value("#{kframework['addon.etc']}") private String etc;
    @Value("#{kframework['addon.language']}") private int language;
    @Value("#{kframework['addon.charset']}") private int charset;
    ...
    {    
        ...
        try {
            ...
            if(charset == 4) {
            	rc = crx.setCharacterEncoding(hd, "utf8");
            } else {
            	rc = crx.setCharacterEncoding(hd, "euc-kr");
            }
...
/*
                else if(crxName.equalsIgnoreCase("RANK")) { //RANK
                    System.arraycopy(crx.getResponseParamValueIntArray(hd, i), 0, arrResult[1], 0, keyword_count);
                    for(String one : arrResult[1]){
                        logger.error("rank:" + one);
                    }
                }
*/
                else if(crxName.equalsIgnoreCase("TAG")) { //TAG
