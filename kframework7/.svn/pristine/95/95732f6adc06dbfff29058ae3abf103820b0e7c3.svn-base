...
import com.konantech.kframework.dao.PersonDao;
...
public class PersonService {
	...
	public ResultListVO search(ParameterVO paramVO) throws Exception {
		ResultListVO result;
		if( useRestFul == true) {
			result = personDao.restSearch(paramVO);
			return result;
		} else if( useBroker == false) {
			result = personDao.submitQuery(paramVO);
			return result;
		}else {
			result = personDao.brokerQuery(paramVO);
			return result;
		}
