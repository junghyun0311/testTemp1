...
public class PersonDao {
	...
	@Value("#{kframework['personHilight']}") private String personHilight;
	...
	public ResultListVO restSearch(ParameterVO param) throws Exception {
		...
		String from = "person.person"; //검색 대상 볼륨,테이블
		...
		int pageSize = paramVO.getPageSize();					// 페이지 사이즈
		if( pageSize == 3 )
			paramVO.setPageSize(4);
		
		String strNmFd = paramVO.getSrchFd();
		
		//영역 검색
		if("title".equals(strNmFd)) {
			//이름만 검색
			queryValue = new String[2][2];
			queryValue[0][0] = "prs_nm_ko";
			queryValue[0][1] = "like";
			queryValue[1][0] = "prs_nm_en";
			queryValue[1][1] = "like";
			
			
			// queryValue값에 맞춰서 재검색과 일반 검색을 처리하여 리턴
			query = dcUtil.makeMasterQuery(queryValue, paramVO);
			...
			queryValue[0][0] = "strl_idx";
			queryValue[0][1] = "like";
			queryValue[1][0] = "strp_idx";
			queryValue[1][1] = "like";
			queryValue[2][0] = strNmFd;
			queryValue[2][1] = "allword";
		...
		searchVO.setHilightFileds(URLEncoder.encode(personHilight, "UTF-8"));
